...
void LoadConfig(char* path, Settings* this)
{
    ...
    if (fstat(fd,&sFile) == -1)
    {
        fprintf(stderr,"Error while getting information about %s\n",path);
        return;
    }
    ...
    struct SetEntry SetEntries[] = {
        ...
        {"INOTIFY_MASK",2,&this->INOTIFY_MASK},{"MoveOffSet",2,&this->MoveOffSet},
        {"WrapScroll",0,&this->WrapScroll},{"JumpScrollValue",2,&this->JumpScrollValue},
        ...
        {"C_Group_7",2,&this->C_Group_7},{"Threads",0,&this->Threads}, {NULL,0,NULL}
    };
