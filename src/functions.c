...
static void godown(csas *cs, const int ws, const int which)
{
    if (cs->ws[ws].visual) {
        for (size_t i = G_S(ws,which); i < G_D(ws,which)->size; i++)
            G_D(ws,which)->xf[i].list[ws] |= 1<<cs->ws[ws].sel_group;
    }
...
void move_to(csas *cs, const int ws, const int which, const char *name)
{
    ...
    for (i = 0; i < G_D(ws,which)->size; i++)
        if (strcmp(G_D(ws,which)->xf[i].name,name) == 0) {
            found = 1;
            break;
        }
    ...
    size_t n = 0;
    if (i > G_S(ws,which)) n = i-G_S(ws,which);
    else if (i < G_S(ws,which)) n = G_S(ws,which)-i;
...
static void GETSIZE(struct xfile *xf, const int fd, const uchar flag)
{
    if ((xf->type&T_GT) == T_DIR) {
        int tfd;
        if ((tfd = openat(fd,xf->name,O_DIRECTORY)) != -1) {
            if (flag&D_F) {
                struct stat sFile;
                stat(xf->name,&sFile);
                xf->size = sFile.st_size;
            }
            else {
                ull size = 0, count = 0;
                get_dirsize(tfd,&count,&size,flag);
                xf->size = (flag&D_C) ? count : size;
            }
            close(tfd);
        }
    }
...
int cmd_getsize(char *src, csas *cs)
{
    ...
    if (ws != cs->current_ws && chdir(G_D(ws,1)->path) != 0)
		return -1;
...
int cmd_f_mod(char *src, csas *cs)
{
    ...
    if (!cs->ws[cs->current_ws].exists) goto END;

    if (ws != cs->current_ws && chdir(G_D(ws,1)->path) != 0)
		return -1;

    if (selected == -2) selected = cs->ws[ws].sel_group;
    if (!target[0]) strcpy(target,".");
    ...
    if (selected == -3) {
        if (
            ...
            for (;;) {
                ...
                else if (si == 'y' || si == 'Y') break;
                else if (si != -1) goto END2;
            }
...
int cmd_load(char *src, csas *cs) {
    ...
    uchar flag = 0;
	werase(cs->win[0]);
	if (s_Win1Enable) werase(cs->win[1]);
	if (s_Win3Enable) werase(cs->win[2]);
...
int cmd_search(char *src, csas *cs)
{
    ...
    if (
        ...
        return 0;
    size_t pos = 0, mul = atoi(cs->typed_keys);
    if (mul == 0) mul = 1;
    int selected = -1;
    int action = -1;
    char temp[1024];
    ...
    switch (action) {
        ...
        case 5:
            ...
            for (size_t i = 0; i < G_D(cs->current_ws,1)->size; i++) {
                if (selected == -1 ? 1 : (G_D(cs->current_ws,1)->xf[i].list[cs->current_ws]&selected)) {
...
int cmd_filter(char *src, csas *cs)
{
    ...
    for (size_t i = 0; i < G_D(cs->current_ws,1)->size; i++) {
        if (!strstr(G_D(cs->current_ws,1)->xf[i].name,src)) {
            temp = G_D(cs->current_ws,1)->xf[i];
            for (size_t j = i; j < G_D(cs->current_ws,1)->size-1; j++)
                G_D(cs->current_ws,1)->xf[j] = G_D(cs->current_ws,1)->xf[j+1];
            G_D(cs->current_ws,1)->xf[G_D(cs->current_ws,1)->size-1] = temp;
